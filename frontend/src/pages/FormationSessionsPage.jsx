...
const FormationSessionsPage = () => {
  ...
  const getImageUrl = (imagePath) => {
    ...
    // Si c'est déjà une URL complète (Cloudinary ou autre), on retourne tel quel
    if (imagePath.startsWith('http')) return imagePath;
    // Sinon, on reconstruit le chemin local
  ...
  useEffect(() => {
    const fetchSessions = async () => {
      try {
        // CORRECTION : utilisation de mode_formation=session au lieu de type=session
              ...
              <SwiperSlide key={session.id}>
                <div className="group relative overflow-hidden rounded-xl shadow-lg bg-white transition-all duration-300 hover:shadow-xl">
                  ...
                  <div className="absolute inset-0 bg-gray-900/80 opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col justify-center items-center p-6 text-center">
                    ...
                    <p className="text-green-500 font-black mb-6 uppercase text-xs">{session.duree}</p>
                    <button 
                      onClick={() => window.location.href = `/inscription/${session.id}`}
                      className="bg-white text-gray-900 px-6 py-3 rounded-lg font-black text-[10px] uppercase tracking-widest hover:bg-green-500 hover:text-white transition-colors"
                    >
